...
start() {
	...
	/sbin/start-stop-daemon --start --make-pidfile --chdir ${TIGASE_HOME} --pidfile $PIDFILE  \
	--chuid $USERNAME:$USERGROUP --exec $JAVA -- $TIGASE_CMD >>${TIGASE_CONSOLE_LOG} 2>&1 &
...
stop() {
        su ${USERNAME} -c "/sbin/start-stop-daemon --stop --quiet  --chdir ${TIGASE_HOME} --pidfile $PIDFILE \
				--chuid $USERNAME:$USERGROUP  --exec $JAVA > /dev/null"
